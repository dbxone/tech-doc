...
fs.writeFile(path.join(__dirname,'../dbx_contract_api.md'), content, function (err) {
    if (err) {
        throw err
    }
...
fs.writeFile(path.join(__dirname,'../dbx_contract_api_EN.md'), enContent, function (err) {
    if (err) {
        throw err
    }
